...
- `components/prompt/PromptInput.tsx` — the AI prompt bar above the table; it passes the `askQuestion` server action to `<form action>` via `useActionState`, so React owns the pending flag and the answer/error state and the form degrades to a plain POST without JS. That last property is fragile and easy to break twice over. First, the action must be passed **directly**: `useActionState` only emits a natively submittable form (`method="POST"` plus the `$ACTION_*` hidden fields carrying the action id and the bound previous state) when it is handed a server reference, so wrapping it in a client function — to catch a failed request, say — silently swaps that for `action="javascript:throw new Error('React form unexpectedly submitted.')"` and the form submits nothing without JS. Second, only `pending` may gate the submit button: a `disabled` that also depends on the input value is baked into the prerendered HTML, so a no-JS visitor could never submit at all (a disabled default button suppresses Enter-key implicit submission too). Verify both in `.next/server/app/index.html` after touching this file — the rendered `<form>` tag and button are the only proof. Empty input is blocked by the native `required` attribute, which works without JS, with `askQuestion`'s own check as the backstop for whitespace-only. Like `Form.tsx` the input is uncontrolled, which means React clears it after each submit — hence the error branch repeats the question back, otherwise a retryable failure (timeout, rate limit) would leave the user retyping. Two requests can never be in flight: the button is disabled while `pending`, which is what makes client-side cancellation unnecessary. The accepted cost of passing the action directly is that a transport-layer failure (offline, proxy 502) rejects inside React's render rather than returning an error state, and there is no error boundary in `app/`. It holds no dataset state and does not touch the query string.

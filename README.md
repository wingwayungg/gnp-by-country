...
The last two rows show the guardrails: a question unrelated to the dataset gets `Irrelevant`, and a question about the right topic but a country the World Bank doesn't publish data for gets `No data` — rather than the model guessing or reaching for outside knowledge.
...
## Technical Features
  ...
  - **AI Question Answering**. A `POST /api/ask` route sends the user's question to Gemini with the entire dataset (pre-sorted by value) passed in as a system instruction, so ranking questions like "top 5" are answered by reading consecutive lines rather than by sorting 176 numbers in-model. Because that instruction is identical on every request and only the question changes, Gemini recognizes text it has already seen and reuses it (implicit context caching), so the repeated part is billed at a discount and the LLM answers faster by reusing internal neural states. Explicit caching is avoided on purpose: it charges by the hour to keep the data in memory, which makes no sense for a low-traffic personal site. The route fails fast and surfaces a clear error rather than a wrong or half-written answer: it aborts after 12 seconds, rejects a truncated response outright instead of returning a cut-off number, and distinguishes a bad API key, a rate limit and a genuine service outage into different error messages. On the client, `PromptInput` aborts a superseded in-flight request so an answer to an old question can never overwrite the answer to a newer one.
